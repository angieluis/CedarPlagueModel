...
### joe's model : transmission prob is cumulative not trajectory
### This has some leaving S class not going anwhere - so at least need to change that. Talk to Joe
SIR.JoesModel=function(t,x,params){
  b=params["b"]          #biting rate of fleas
  b1=params["b1"]        #biting rate of blocked fleas
  tep=params["tep"]      #proportion of INFECTIOUS bites from early-phase fleas
  tpb=params["tpb"]      #proportion of INFECTIOUS bites from partially blocked fleas
  ...
  prob.transmissionEP <-  1 - (1-pep)^(Iep*b*S/H)
  prob.transmissionPB <-  1 - (1-ppb)^(Ipb*b*S/H)
  prob.transmissionB <-  1 - (1-pb)^(Ib*b1*S/H)
  # if those are prob transmission that should affect rate leaving S not whether go to L or E
  
  dS = -S*(b*Iep*pep + b*Ipb*ppb + b1*Ib*pb)/H - mu*S   #Susceptible Rodent Pop
  dL = S*(prob.transmissionEP*tep + prob.transmissionPB*tpb + prob.transmissionB*tb)/H - sigma*L - mu*L  #infection of host, not yet infectious
  dI = sigma*L - I*(mu+epsilon)  #infectious rodent hosts
  dE = S*(prob.transmissionEP*(1-tep) + prob.transmissionPB*(1-tpb) + prob.transmissionB*(1-tb))/H - E*(mu+gamma) #exposed rodents (non-infectious)
  dR = gamma*E - R*mu  #recovery of rodent
  dr = mu*H + epsilon*I  #dead rodents
  Id = epsilon*I
